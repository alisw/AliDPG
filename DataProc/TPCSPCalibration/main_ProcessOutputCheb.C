...
Bool_t main_ProcessOutputCheb(TString filesToProcess, Int_t startRun, Int_t endRun, const char* ocdbStorage, 
  ...
  */
  ...
  while ((nextfile=next())) {
    ...
    if (corr) { 
      AliTPCChebCorr* c =  const_cast<AliTPCChebCorr*>(dcalibRes->GetChebCorrObject());
      if (!c) {
    ...
    if (dist) { // 
      AliTPCChebDist* d =  const_cast<AliTPCChebDist*>(dcalibRes->GetChebDistObject());
      if (!d) {
	::Error("ProcessOutput","Did not find %s Cheb.parm in %s",corr ? "Correction":"Distortion" ,snextfile.Data());
	//PrintProcStatus(kStatusFail);
	//exit(1);
  ...
  if (badStatJobs) {
    ...
    if (corr) {
      printf("Corrections\n");
      acorr->Print();
      TString argcorr = TString::Format("(reinterpret_cast<TObjArray*>(%p))", acorr);
      if (!gInterpreter->ProcessLine(TString::Format("CreateCorrMapObjTime(%s, %d, %d, \"%s\")", argcorr.Data(), startRun, endRun, ocdbStorage))) status = kStatusFail;      
    }
    if (dist && adist->GetEntries()) {
      printf("Distortions\n");
      adist->Print();
      TString argdist = TString::Format("(reinterpret_cast<TObjArray*>(%p))", adist);
      if (!gInterpreter->ProcessLine(TString::Format("CreateCorrMapObjTime(%s, %d, %d, \"%s\")", argdist.Data(), startRun, endRun, ocdbStorage))) status = kStatusFail;
    }
