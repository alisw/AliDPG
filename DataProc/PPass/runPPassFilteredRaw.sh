...
if [ -f cloneraw.C ]; then
    echo "Use cloneraw.C macro passed as input"
else
    echo "Use cloneraw.C macro from AliDPG"
    cp $ALIDPG_ROOT/DataProc/Common/cloneraw.C .
fi
...
# First, we need to extract the interesting raw data
echo ""
echo "running the following cloneraw.C macro:"
cat cloneraw.C
echo ""
echo "" >&2
echo "cloneraw.C" >&2
...
echo "cloneraw:   delta = $delta, timeUsed so far = $timeUsed"
echo "cloneraw:   delta = $delta, timeUsed so far = $timeUsed" >&2
echo "*! Exit code of cloneraw.C: $exitcode"
if [ $exitcode -ne 0 ]; then
    echo "cloneraw.C exited with code $exitcode" > validation_error.message
    exit 10
fi
