...
//______________________________________________________________________________
void main_AODtrain(Int_t merge=0)
{
  ...
  gSystem->SetBuildDir(gSystem->pwd(), kTRUE);
   ...
   if (merge) {
     TChain* chain = 0x0;
      ...
      mgr->StartAnalysis("gridterminate",chain);
      return;
   }   
   // Run the analysis                                                                                                                     
   //
...
void AddAnalysisTasks(const char *cdb_location)
{
  ...
  //
   ...
   if (useTender) {
      ...
      AliTender *tender = AddTaskTender(useV0tender);
...
// ********** PWG3 wagons ******************************************************           
  ...
  if (iPWGHFvertexing) 
  {
   ...
   if (iPWGDQJPSIfilter && (iCollision==kpp)) {
     AliAnalysisTaskDielectronFilter *taskJPSIfilter = AddTaskJPSIFilter();
